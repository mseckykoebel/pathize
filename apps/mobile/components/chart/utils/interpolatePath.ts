...
/**
 * Copy of https://github.com/coinjar/react-native-wagmi-charts/blob/master/src/charts/line/utils/interpolatePath.ts
 */
 ...
 */
function decasteljau(points: number[][], t: number): object {
  'worklet';
  ...
  for (let j = 0; j <= n; j++) {
    left[j] = points[j].slice();
    right[j] = points[n - j].slice();
  }

  for (let k = 1; k <= n; k++) {
    for (let j = 0; j <= n - k; j++) {
      left[j][0] = (1 - t) * left[j][0] + t * left[j + 1][0];
      left[j][1] = (1 - t) * left[j][1] + t * left[j + 1][1];

      let rIdx = n - j;
      right[rIdx][0] = (1 - t) * right[rIdx - 1][0] + t * right[rIdx][0];
      right[rIdx][1] = (1 - t) * right[rIdx - 1][1] + t * right[rIdx][1];
    }
  }

  return {left: left.slice(0, n + 1), right: right.reverse().slice(0, n + 1)};
}
 ...
 */
function pointsToCommand(points: number[][]): object {
  'worklet';
  ...
  command.x = lastPoint[0];
  command.y = lastPoint[1];
...
function splitCurveAsPoints(
  ...
  for (let i = 0; i < segmentCount - 1; i++) {
    ...
    segments[i] = split.left;
    remainingCurve = split.right;
  }

  // last segment is just to the end from the last point
  segments[segmentCount - 1] = remainingCurve;
 ...
 */
function commandToString(command: object): string {
  'worklet';
 ...
 */
function convertToSameType(aCommand: object, bCommand: object): object {
  'worklet';
 ...
 */
function extend(
  commandsToExtend: object[],
  referenceCommands: object[],
  excludeSegment: Function,
): object[] {
  'worklet';
  ...
  const extended = countPointsPerSegment.reduce((ex, segmentCount, i) => {
    ...
    if (i === commandsToExtend.length - 1) {
      ...
      return ex.concat(lastCommandCopies);
    }
...
export function interpolatePathCommands(
  aCommandsInput: object[],
  bCommandsInput: object[],
  excludeSegment: Function,
): Function {
  'worklet';
