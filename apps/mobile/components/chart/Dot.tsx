...
export function LineChartDot({
  ...
  const {isActive, mapPointsToFixedRange} = useLineChart();
  const {
    parsedPath,
    pointWidth,
    pathWidth: width,
  ...
  const {isInactive: _isInactive} = useContext(LineChartPathContext);
    ...
    () =>
      withTiming(
        mapPointsToFixedRange
          ? mapToMaxVal(pointWidth + at, width)
          : pointWidth + at,
      ),
    [at, pointWidth],
  );
