...
import {getBarColorWithResting} from '../utils/getBarColor';

const START_X = 0;
const FINISH_X = 24 * 60;

export const PacingBarChartWithResting = ({
  chartWidth,
  chartHeight,
  pointsToday,
  minHr,
    ...
    <>
                ...
                if (item.y !== undefined) {
                  return (
                    <Line
                      ...
                      x2={scaleX(item.x)}
                      y2={scaleY(minHr)}
                      stroke={getBarColorWithResting(
                        limit - (limit - minHr) * 0.5,
                        item.y,
                        limit,
                        limit + (maxHr - limit) * 0.5,
                      )}
                      strokeWidth={1}
                    />
                  );
      ...
      <View style={[tw`flex flex-row items-center justify-center mt-6`]}>
        <View style={[tw`flex flex-row items-center ml-6`]}>
          <View
            style={[tw`w-3 h-3 rounded-sm ml-1`, {backgroundColor: '#7DD3FC'}]}
          />
          <View
