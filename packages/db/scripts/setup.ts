...
async function setupEnv() {
  const { publicJwk: publicEncKey, privateJwk: privateEncKey } =
    await generateAndExportKeyPair("ECDH-ES+A256KW");

  setEnvValue("PRIVATE_ENCRYPTION_KEY", JSON.stringify(privateEncKey));
  setEnvValue("PUBLIC_ENCRYPTION_KEY", JSON.stringify(publicEncKey));
